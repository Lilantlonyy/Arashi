...
module.exports = async (Arashi, m, commands, chatUpdate) => {
  try {
    ...
    const metadata = m.isGroup ? await Arashi.groupMetadata(from) : {};
    ...
    const groupAdmin = m.isGroup
      ...
      : [];
    const botNumber = await Arashi.decodeJid(Arashi.user.id);
    ...
    global.suppL = "https://cutt.ly/ArashiBotSupport";
    ...
    async function doReact(emoji) {
      ...
      await Arashi.sendMessage(m.from, reactm);
    }
    ...
    //if (body.startsWith(prefix) && !icmd)  return Arashi.sendMessage(m.from, { text: "Baka no such command" });
    ...
    if (isAntilinkOn && m.isGroup && !isAdmin && !isCreator && isBotAdmin) {
      const linkgce = await Arashi.groupInviteCode(from);
