...
exports.serialize = (Arashi, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && Arashi.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(Arashi.user && Arashi.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        Arashi.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        Arashi.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => Arashi.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? Arashi.sendFile(chatId, text, "file", "", m, { ...options })
      : Arashi.sendText(chatId, text, m, { ...options });
